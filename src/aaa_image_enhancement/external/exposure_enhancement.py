...
def create_spacial_affinity_kernel(spatial_sigma: float, size: int = 15):
    ...
    for i in range(size):
        for j in range(size):
            kernel[i, j] = np.exp(
                -0.5
                * (distance.euclidean((i, j), (size // 2, size // 2)) ** 2)
                / (spatial_sigma**2)
            )
...
def compute_smoothness_weights(
    L: np.ndarray, x: int, kernel: np.ndarray, eps: float = 1e-3
):
...
def refine_illumination_map_linear(
    L: np.ndarray, gamma: float, lambda_: float, kernel: np.ndarray, eps: float = 1e-3
):
    ...
    A = Id + lambda_ * F
    L_refined = spsolve(csr_matrix(A), L_1d, permc_spec=None, use_umfpack=True).reshape(
        (n, m)
    )
...
def correct_underexposure(
    im: np.ndarray, gamma: float, lambda_: float, kernel: np.ndarray, eps: float = 1e-3
):
...
def enhance_image_exposure(
    ...
    if dual:
        ...
        over_corrected = 1 - correct_underexposure(
