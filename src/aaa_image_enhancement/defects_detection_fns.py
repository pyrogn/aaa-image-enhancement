...
def is_dark_color(image: ImageConversions, threshold: int = 50) -> dict[DefectNames, bool]:
...
def is_dark_histogram(image: ImageConversions, dark_threshold: int = 50, dark_proportion: float = 0.5) -> dict[DefectNames, bool]:
...
def is_dark_threshold(image: ImageConversions, brightness_threshold: int = 50, dark_area_proportion: float = 0.5) -> dict[DefectNames, bool]:
    grayscale_image = image.to_grayscale()
    dark_pixels = np.sum(grayscale_image < brightness_threshold)
    total_pixels = grayscale_image.size
    return {DefectNames.DARK_THRESHOLD: (dark_pixels / total_pixels) > dark_area_proportion}


def is_dark_local_contrast(image: ImageConversions, window_size: int = 15, dark_threshold: int = 50, dark_proportion: float = 0.5) -> dict[DefectNames, bool]:
    grayscale_image = image.to_grayscale()
    ...
    total_pixels = grayscale_image.size
    return {DefectNames.DARK_LOCAL_CONTRAST: (dark_pixels / total_pixels) > dark_proportion}


def is_dark_adaptive_threshold(image: ImageConversions, max_value: int = 255,
                               adaptive_method: int = cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                               threshold_type: int = cv2.THRESH_BINARY, block_size: int = 11, C: int = 2,
                               dark_proportion: float = 0.5) -> dict[DefectNames, bool]:
    grayscale_image = image.to_grayscale()
    adaptive_thresh = cv2.adaptiveThreshold(grayscale_image, max_value, adaptive_method, threshold_type, block_size, C)
    dark_pixels = np.sum(adaptive_thresh == 0)
    total_pixels = adaptive_thresh.size
    return {DefectNames.DARK_ADAPTIVE_THRESHOLD: (dark_pixels / total_pixels) > dark_proportion}


def is_dark_v_channel(image: ImageConversions, threshold: int = 50, dark_proportion: float = 0.5) -> dict[
    DefectNames, bool]:
...
def is_dark_edges(image: ImageConversions, edge_threshold1: int = 100, edge_threshold2: int = 200,
                  brightness_threshold: int = 50, dark_proportion: float = 0.5) -> dict[DefectNames, bool]:
    grayscale_image = image.to_grayscale()
...
def is_dark_blocks(image: ImageConversions, block_size: int = 32, dark_threshold: int = 50,
                   dark_proportion: float = 0.5) -> dict[DefectNames, bool]:
    grayscale_image = image.to_grayscale()
    ...
    return {DefectNames.DARK_BLOCKS: (block_dark_count / total_blocks) > dark_proportion}
